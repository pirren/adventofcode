...
let [year] = [2016]; // todo: default

let args = _.compact([process.argv[2]])
if (args.length === 1) {
    [year] = args
}
...
const fetchLeaderboardDataAsync = async () => {
...
const fetchAndCacheAsync = async () => {
    let cache = await fetchLeaderboardDataAsync();
...
if (cache.length === 0) {
    cache = await fetchAndCacheAsync()
} 
...
const table = ttyTable(header, playerData);
